Describe "Mozilla Build v2" -Skip:(Assert-IsBuilder) {
    BeforeDiscovery {
        $Hiera = $Data.Hiera
    ...
    Context "Pip" {
        ...
        # }
        It "Pip.ini exists" {
            Test-Path "$ENV:ProgramData\pip\pip.ini" | Should -Be $true
        }
        It "Pip.ini does not configure legacy download-cache" {
            Get-Content "$ENV:ProgramData\pip\pip.ini" -Raw |
            Should -Not -Match "(?im)^\s*download-cache\s*="
        }
        It "PIP_DOWNLOAD_CACHE environment variable is absent" {
            [Environment]::GetEnvironmentVariable("PIP_DOWNLOAD_CACHE", "Machine") |
            Should -BeNullOrEmpty
            $ENV:PIP_DOWNLOAD_CACHE | Should -BeNullOrEmpty
        }
        It "Legacy pip-cache folders are absent" {
            Test-Path "C:\pip-cache" | Should -Be $false
            Test-Path "D:\pip-cache" | Should -Be $false
        }
    }
...
Describe "Mozilla Build v2 - Builder" -Skip:(Assert-IsTester) {
    BeforeDiscovery {
        $Hiera = $Data.Hiera
        C:\mozilla-build\python3\python.exe -m pip freeze --all > C:\requirements.txt
    }
