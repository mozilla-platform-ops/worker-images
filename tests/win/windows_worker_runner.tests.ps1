...
Describe "Taskcluster" {
    ...
    Context "Generic Worker" {
        ...
        It "Generic Worker Version is correct" {
            $version = Start-Process -FilePath "C:\generic-worker\generic-worker.exe" -ArgumentList "--short-version" -Wait -NoNewWindow
            $version | Should -be $generic_worker_version
        }
    }
    Context "Worker Runner" {
        ...
        It "Worker Runner Version is correct" {
            $version = Start-Process -FilePath "C:\worker-runner\start-worker.exe" -ArgumentList "--short-version" -Wait -NoNewWindow
            $version | Should -be $worker_runner_version
        }
    }
    Context "Proxy" {
        It "Proxy exists" {
        ...
        It "Proxy version is correct" {
            $version = Start-Process -FilePath "C:\generic-worker\taskcluster-proxy.exe" -ArgumentList "--short-version" -Wait -NoNewWindow
            $version | Should -be $proxy_version
        }
    }
    Context "Livelog" {
        It "Livelog exists" {
        ...
        It "Livelog version is correct" {
            $version = Start-Process -FilePath "C:\generic-worker\livelog.exe" -ArgumentList "--short-version" -Wait -NoNewWindow
            $version | Should -be $livelog_version
        }
    }
}
