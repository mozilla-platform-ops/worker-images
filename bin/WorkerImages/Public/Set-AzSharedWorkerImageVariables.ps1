function Set-AzSharedWorkerImageVariables {
    ...
    if (-not $env:RUNNER_TEMP) { throw 'Set RUNNER_TEMP to a staging directory before preparing Azure uploads.' }
    $staging = Join-Path $env:RUNNER_TEMP ([guid]::NewGuid().ToString('N'))
    ...
    # GitHub clears RUNNER_TEMP after the job, including failed builds.
}
