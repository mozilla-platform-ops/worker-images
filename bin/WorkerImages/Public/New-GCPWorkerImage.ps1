function New-GCPWorkerImage {
    ...
    # Choose YAML + HCL paths. Default preserves previous behavior.
    ...
    ## Initialize Packer plugins for the selected HCL
    Write-Host "packer init $PackerHCLPath"
    packer init $PackerHCLPath

    ## Run Packer on specific builder (names must match the source blocks in HCL)
