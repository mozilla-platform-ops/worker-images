function New-GCPWorkerImage {
    ...
    # Secrets for packer provisioner env
    ...
    # Image name: date suffix unless *alpha*
    ...
    if ($Team -and $Team -ieq "tceng") {
        # tceng HCL has a single source; no --only to avoid builder-name coupling
        Write-Host "packer build -force $PackerHCLPath"
        packer build -force $PackerHCLPath
    } else {
        # Preserve existing behavior for classic HCLs
