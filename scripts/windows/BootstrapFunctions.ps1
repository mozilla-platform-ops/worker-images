...
function Install-AzPrerequ {
    ...
    process {
        ...
        $null = New-Item -path $work_dir -ItemType "directory" -ErrorAction SilentlyContinue
        #if ($ps_ver -le 5) {
...
Function Set-AzRoninRepo {
    param (
        ...
        [string] $deploymentID = ((((Invoke-WebRequest -Headers @{'Metadata' = $true } -UseBasicParsing -Uri ('http://169.254.169.254/metadata/instance?api-version=2019-06-04')).Content) | ConvertFrom-Json).compute.tagsList | Where-Object { $_.name -eq ('deploymentId') })[0].value
    )
    begin {
        Write-Log -message ('{0} :: begin - {1:o}' -f $($MyInvocation.MyCommand.Name), (Get-Date).ToUniversalTime()) -severity 'DEBUG'
    }
