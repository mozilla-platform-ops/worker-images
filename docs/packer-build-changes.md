...
in that audit are hypotheses; these changes have not been timed in cloud builds.
No production image versions or ronin_puppet pins are bumped here.

## Terminology

- **Packer templates** are `.pkr.hcl` files; **Packer sources** are their named source definitions.
- **Image configs** are YAML files under `config/`; `config/windows_production_defaults.yaml` supplies shared defaults.
- **Build workflows** live under `.github/workflows/`; the **composite action** is `.github/actions/packer-build/action.yml`.
- **Runner helpers** run outside the image-build VM; **provisioning scripts** run inside it. PowerShell functions are referred to by their exact names.
- Issue links below refer to `mozilla-platform-ops/worker-images`, not similarly numbered issues or PRs in `mozilla-releng/fxci-config`.
...
| W4 / [worker-images#17](https://github.com/mozilla-platform-ops/worker-images/issues/17) | Remove Az PowerShell initialization and the tceng no-op removal step. Azure CLI login is now needed for the requested SKU preflight, as well as the replication job; it is not an unused step anymore. Packer still performs its own OIDC exchange. |
...
The four FXCI Azure dispatch workflows share the concrete image-build job in
`.github/workflows/build-azure-image.yml`, without adding Azure logic to the
Packer action. RelSRE authorization and config/trust validation happen before
Azure login. Each caller passes only its four selected Azure secrets; the build
needs `contents: read` and `id-token: write`. Matrices, region checks, release-note
publication, and integration/replication gates remain in the entrypoints.
Successful-build filtering accepts both flat GCP job names and Azure reusable
job names (`Build <config> / Packer`).

All eight Packer build workflows use `.github/actions/packer-build` with a
Packer template and optional build selector. The action runs `packer init`,
`packer validate`, and `packer build`, stopping on any native failure. It has no
cloud/image-family dispatcher or dependency on this repository's build helpers.
The callers own authentication, config validation/preparation, GCP SDK setup,
replication, and publishing. `ci/test-packer-action.ps1` checks template files and
directories, selectors, force behavior, argument safety, and all three failure
boundaries. `ci/test-build-helpers.ps1` separately checks config preparation and
the environment/artifact handoff without invoking Packer.

Azure preparation retains upload ZIPs in `RUNNER_TEMP` for the later Packer step;
the runner clears them after the job. A deferred replication request is prepared
before the build, but its artifact is uploaded only after Packer succeeds.
...
- Remove the unused root Azure nonsig source/variables/provisioner. tceng retains
  its own templates. `Set-AzWorkerImageVariables` rejects the removed root nonsig route.
...
- `Scan-NUCHealth.ps1` and `UninstallFirefox.ps1` share `Invoke-RemoteScriptBatch.ps1`.
...
- Remove unreachable non-tceng branches from `Set-AzWorkerImageVariables`.
...
- **[worker-images#699](https://github.com/mozilla-platform-ops/worker-images/issues/699):** enable pinned PSScriptAnalyzer 1.25.0 checks for all tracked production
  ...
  plaintext password parameter from `OS-deploy.ps1`; forward its development script
...
- **[worker-images#17](https://github.com/mozilla-platform-ops/worker-images/issues/17):** the shared Azure build functions query `az vm list-skus` before Packer
...
- **[worker-images#14](https://github.com/mozilla-platform-ops/worker-images/issues/14):** the region check reuses fxci-config's own variant/alias resolver under
...
- **External blocker:** feature-branch Azure OIDC subjects need identity-owner
  approval/configuration outside this repo. These changes neither grant access nor
...
1. Obtain an approved Azure branch-build identity subject. Do not weaken the
   authorization or trusted/untrusted subscription separation to test these changes.
