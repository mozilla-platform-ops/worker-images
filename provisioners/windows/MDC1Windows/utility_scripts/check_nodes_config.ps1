function Set-LocalPXE {
    ...
    # Escape backticks and quotes for use in a heredoc to be passed to echo
    $remoteScript = @'
function Write-Log {
  param (
    [string] $message,
...
'@
    ...
    $encodedScript = $remoteScript -replace '"', '`"'  # escape quotes
    $echoCommand = "echo `"$encodedScript`" > C:\SetPXE.ps1"

    Invoke-SSHCommand -Command $echoCommand -NodeName $NodeName
