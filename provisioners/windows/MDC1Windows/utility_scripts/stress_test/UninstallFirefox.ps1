...
. "$PSScriptRoot/Invoke-RemoteScriptBatch.ps1"
$batchArgs = @{
    Parallel = $max_parallel
    User = $ssh_user
    Payload = $diagPayload
    NamePrefix = 'uninstall_firefox'
    TimeoutMs = 300000
    ServerAliveCountMax = 4
    DescribeResult = { param($short, $data) "[$short] $($data.Action)  before=$($data.FF_PresentBefore)  managed=$($data.Choco_Managed)  after=$($data.FF_PresentAfter)" }
}
...
Write-Host "==== PASS 1 ===="
$results = @{}
$pass1 = Invoke-RemoteScriptBatch -Fqdns $targets @batchArgs
...
while (-not $dry_run -and ($results.GetEnumerator() | Where-Object { $_.Value._s -eq 'busy' } | Measure-Object).Count -gt 0) {
    ...
    $r = Invoke-RemoteScriptBatch -Fqdns $busyTargets @batchArgs
...
while ($pass -le $ssh_max_retries) {
    ...
    $r = Invoke-RemoteScriptBatch -Fqdns $failed @batchArgs
    foreach ($k in $r.Keys) { $results[$k] = $r[$k] }
    $pass++
}
